...
FILENAME_CONF = "classification.toml"
CONF_PATH = os.path.join(FILE_DIR, FILENAME_CONF)


class ClassificationAdaptor(Adaptor):
    ...
    def publish_report(self: Self, identifier: str,
        ...
        """ Write the validation report (as N-Triples) for
            the state graph with the provided identifier and
            ground truth label to the standard output
    ...
    def translate(self: Self, data: dict[str, Any])\
        ...
        if not (isinstance(graph_label, int) or
                isinstance(graph_label, list) and len(graph_label) > 0):
            logging.debug("Missing graph identifier in data package")
            return data_translated
